...
export class CheckIfUnsafe extends Effect.Service<CheckIfUnsafe>()(
	...
	{
		effect: genLogger('studiocms/virtuals/auth/utils/unsafeCheck/CheckIfUnsafe.effect')(
			function* () {
				/**
				...
				const username = (val: string) =>
					pipeLogger('studiocms/virtuals/auth/utils/unsafeCheck/CheckIfUnsafe.username')(
						Effect.try({
				...
				const password = (val: string) =>
					pipeLogger('studiocms/virtuals/auth/utils/unsafeCheck/CheckIfUnsafe.password')(
						Effect.try({
