...
import { StorageManagerResolver } from '../context';
 ...
 */
export const resolveStorageManagerUrls = <F>(obj: F, attributes: keyof F | (keyof F)[]) =>
	Effect.gen(function* () {
		// Get the storage manager resolver from the context
		const smResolver = yield* StorageManagerResolver;
