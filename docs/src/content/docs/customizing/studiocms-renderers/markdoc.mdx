...
3. Add the MarkDoc React renderer to your configuration:
    ...
    <TabItem label="astro.config.mjs">
        ...
        ```ts twoslash title="astro.config.mjs" {17} ins={5-6, 14, 19}
    ...
    <TabItem label="studiocms.config.mjs">
        ...
        ```ts twoslash title="astro.config.mjs" ins={5, 13}
        import { defineConfig } from 'astro/config';
        import db from '@astrojs/db';
        import node from '@astrojs/node';
        import studioCMS from 'studiocms';
        ...
        ```ts twoslash title="studiocms.config.mjs" {7} ins={2, 9}
...
<ReadMore>For more information on how to configure the MarkDoc renderer, see the [MarkDoc Configuration](/config-reference/renderer-config/markdoc-config/) documentation.</ReadMore>
