...
To define a StudioCMS Plugin, you need to create an object that conforms to the `StudioCMSPlugin` type. This object should contain the following properties:
...
export const myPlugin = (options: Options) => definePlugin({
  ...
  integration: myIntegration(options),
});
```